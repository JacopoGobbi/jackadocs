...
object HTMLToMarkdown {
  ...
  private def convertBlocks(ns:NodeSeq):Seq[MDBlock] = {
    @tailrec def combineStrayInlines(s:Seq[NodeSeq], soFar:Vector[MDInline]=Vector()):Seq[MDBlock] = s match {
  ...
  private def convertSingleBlock:PartialFunction[NodeSeq,MDBlock] = {
    ...
    case e:Elem if e.label == "code" || e.label == "tt" =>
      ...
      MDCodeFence(infoString, inlineTexts.toIndexedSeq)
  ...
  @tailrec private def extractList(nodes:Seq[NodeSeq], ordered:Boolean, soFar:Vector[Seq[MDBlock]]=Vector()):MDList = nodes match {
