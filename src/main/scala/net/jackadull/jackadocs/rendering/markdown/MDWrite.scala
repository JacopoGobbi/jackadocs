...
object MDWrite {
  def apply(blocks:Iterable[MDBlock], out:Appendable):Unit = {
    ...
    def renderBlocks(blocks:Iterable[MDBlock], writer:LineWriter):LineWriter =
    ...
    def renderInlines(inlines:Iterable[MDInline], writer:LineWriter):LineWriter =
  ...
  private final case class LineWriterToAppendable(out:Appendable) extends LineWriter {
    override def append(str:String):LineWriterToAppendable = {out append str; this}
    override def append(ch:Char):LineWriterToAppendable = {out append ch; this}
    override def nextLine():LineWriterToAppendable = {out append '\n'; this}
    override def popLinePrefix():LineWriter = sys error "no line prefix pushed"
  }
  private final case class WithLinePrefix(prefix:String, inner:LineWriter, atBeginningOfLine:Boolean=true) extends LineWriter {
    override def append(str:String):WithLinePrefix =
    ...
    override def append(ch:Char):WithLinePrefix =
      if(atBeginningOfLine) copy(inner = inner append prefix append ch, atBeginningOfLine = false)
      else {
        val newInner = inner append ch
        if(newInner ne inner) copy(inner = newInner) else this
      }
    override def nextLine():WithLinePrefix = copy(inner = inner.nextLine(), atBeginningOfLine = true)
    override def popLinePrefix():LineWriter = inner
  }
  private final case class WithLinePrefix2(nextLinePrefix:String, linesThereafter:String, inner:LineWriter) extends LineWriter {
    override def append(str:String):LineWriter =
      if(str.isEmpty) this
    ...
    override def nextLine():WithLinePrefix =
  ...
  private final case class WithoutLeadingWhitespace(inner:LineWriter) extends LineWriter {
    override def append(str:String):LineWriter = if(str.isEmpty) this else removeLeadingWhitespace(str) match {
      case Some(noWs) if noWs.isEmpty => this
      case Some(noWs) => inner append noWs
      case None => inner append str
    }
    override def append(ch:Char):LineWriter = if(isWhitespace(ch)) this else inner append ch
    override def nextLine():LineWriter = inner.nextLine()
    override def popLinePrefix():LineWriter = inner.popLinePrefix()
